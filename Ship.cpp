...
Ship::Ship(std::string name) : shipName(name)
{
	...
	for (int i = 0; i < 3; i++) // making three human crew members
		addCrewMember("human");
...
void Ship::setCrewCurrent(int num, COORD current) { crew[num].setCurrent(current); }
void Ship::setCrewDestination(int num, COORD destination) { crew[num].setDestination(destination); }
...
string Ship::roomType(string str) {
	...
	const int length = 2;
	string arr1[length] = { "e" , "c"};
	string arr2[length] = { " " , "c"};
...
void Ship::printShip() {
	...
	for (int x = shipMapWidth - 1; x >= 0; x--) {
		for (int z = 0; z < count; z++) {
			for (int y = 0; y < shipMapLength; y++) {
				if (flipMap[x][y] == "empty") {
					cout << "         ";
				}
				else if (flipMap[x][y] == "block") {
					cout << "[][]|[][]";
				}
				else {
					if (z == 0) {

						if (flipMap[x][y].substr(1, 1) == "d") {
							std::cout << "|--#-#--|";
						}
						else if (flipMap[x][y].substr(1, 1) == "o") {
							std::cout << "|--# #--|";
						}
						else if (flipMap[x][y].substr(1, 1) == "w") {
							std::cout << "|-------|";
						}
						else if (flipMap[x][y].substr(1, 1) == "n") {
							std::cout << "|       |";
						}
						...
						if ((flipMap[x][y].substr(0, 1) == "d" || flipMap[x][y].substr(0, 1) == "o") && (flipMap[x][y].substr(2, 1) == "d" || flipMap[x][y].substr(2, 1) == "o")) {
							std::cout << "#       #";
						}
						else if ((flipMap[x][y].substr(0, 1) == "d" || flipMap[x][y].substr(0, 1) == "o") && flipMap[x][y].substr(2, 1) == "w") {
							std::cout << "#       |";
						}
						else if ((flipMap[x][y].substr(0, 1) == "d" || flipMap[x][y].substr(0, 1) == "o") && flipMap[x][y].substr(2, 1) == "n") {
							std::cout << "#        ";
						}

						else if ((flipMap[x][y].substr(0, 1) == "w" && flipMap[x][y].substr(2, 1) == "d") || flipMap[x][y].substr(2, 1) == "o") {
							std::cout << "|       #";
						}
						else if (flipMap[x][y].substr(0, 1) == "w" && flipMap[x][y].substr(2, 1) == "w") {
							std::cout << "|       |";
						}
						else if (flipMap[x][y].substr(0, 1) == "w" && flipMap[x][y].substr(2, 1) == "n") {
							std::cout << "|        ";
						}

						else if ((flipMap[x][y].substr(0, 1) == "n" && flipMap[x][y].substr(2, 1) == "d") || flipMap[x][y].substr(2, 1) == "o") {
							std::cout << "        #";
						}
						else if (flipMap[x][y].substr(0, 1) == "n" && flipMap[x][y].substr(2, 1) == "w") {
							std::cout << "        |";
						}
						else if (flipMap[x][y].substr(0, 1) == "n" && flipMap[x][y].substr(2, 1) == "n") {
							std::cout << "         ";
						}
						...
						string s = flipMap[x][y].substr(4, 1);
						string temp = " ";
						if ((flipMap[x][y].substr(0, 1) == "d" || flipMap[x][y].substr(0, 1) == "w") && (flipMap[x][y].substr(2, 1) == "d" || flipMap[x][y].substr(2, 1) == "w")) {
							temp = "|   " + roomType(s) + "   |";
						}
						else if ((flipMap[x][y].substr(0, 1) == "d" || flipMap[x][y].substr(0, 1) == "w") && (flipMap[x][y].substr(2, 1) == "o" || flipMap[x][y].substr(2, 1) == "n")) {
							temp = "|   " + roomType(s) + "    ";
						}
						else if ((flipMap[x][y].substr(0, 1) == "o" || flipMap[x][y].substr(0, 1) == "n") && (flipMap[x][y].substr(2, 1) == "d" || flipMap[x][y].substr(2, 1) == "w")) {
							temp = "    " + roomType(s) + "   |";
						}
						else if ((flipMap[x][y].substr(0, 1) == "o" || flipMap[x][y].substr(0, 1) == "n") && (flipMap[x][y].substr(2, 1) == "o" || flipMap[x][y].substr(2, 1) == "n")) {
							temp = "    " + roomType(s) + "    ";
						}
						...
						if (flipMap[x][y].substr(3, 1) == "d") {
							std::cout << "|--#-#--|";
						}
						else if (flipMap[x][y].substr(3, 1) == "o") {
							std::cout << "|--# #--|";
						}
						else if (flipMap[x][y].substr(3, 1) == "w") {
							std::cout << "|-------|";
						}
						else if (flipMap[x][y].substr(3, 1) == "n") {
							std::cout << "|       |";
						}
					}
				}
